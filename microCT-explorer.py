...
with tabCitation:
    st.write('The XCT-Explorer is a graphic user interface designed to be an intuitive and interactive tool to help plan CT experiments according to a step-by-step protocol (see **instructions** tab). This should allow for an interactive balancing of various scanning parameters and for a first assessment of the feasibility of experiments. Details about the protocol and the equations that link the various parameters can be found in "citation".')
    st.subheader ('Citation')
    st.write ('If you find XCT-explorer useful for the planning of your experiment, please cite the publication:blue[https://doi.org/10.1016/j.tmater.2024.100041].') 
    ...
    st.write ('For simplicity, some advanced options are not considered. Always interpret the results critically and discuss your assessment with a CT expert. Additional features can be found in the advanced mode of the app: https://xct-explorer-advanced-v1p4.streamlit.app')
...
allPhases = [col for col in database.columns if col != 'Energy (kV)']
############################################## state variables ######################################################    
